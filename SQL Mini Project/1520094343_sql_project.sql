...
SELECT firstname, surname, MAX( joindate ) AS Latest
FROM `Members`
...
the member name. */
SELECT DISTINCT f.name as facilityname, CONCAT( m.firstname, ' ', m.surname ) AS fullname
FROM Members m
...
SELECT fullname, facilityname, cost
FROM (
SELECT f.name AS facilityname, CONCAT( m.firstname, ' ', m.surname ) AS fullname,
CASE
WHEN m.memid =0
